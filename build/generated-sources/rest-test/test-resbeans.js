...
function TestSupport() {
    this.wadlDoc = null;
    this.wadlURL = '';
    this.wadlErr = 'WADL にアクセスできません。RESTful アプリケーションを再起動し、このページを再表示してください。';
    ...
    this.viewIds = [
        { "id" : "table" , "name":"表形式ビュー", "type":"tableContent"}, 
        { "id" : "raw" , "name":"raw データのビュー", "type":"rawContent"}, 
        { "id" : "structure" , "name":"サブリソース", "type":"structureInfo"},
        { "id" : "header" , "name":"ヘッダー", "type":"headerInfo"},
        { "id" : "monitor" , "name":"HTTP モニター", "type":"monitorContent"}];
...
TestSupport.prototype = {    
    ...
    initFromWadl : function () {
        ...
        if(wDoc != null) {
            ...
            this.updatepage('content', '<span class=bld>ヘルプページ</span><br/><br/><p>WADL にアクセスできません。REST アプリケーションを再起動し、このページを再表示してください。</p><p>このエラーがまだ表示され、Firebug プラグインをインストールした Firefox からこのページにアクセスしている場合は、<br/>ローカルファイルに対して Firebug を無効にする必要があります。無効にするには、Firefox のメニューバーから<br/>「ツール」>「Firebug」>「Disable Firebug for Local Files」を選択します。</p>');
        }            
    },
    ...
    getMethodMimeTypeCombo : function (resource) {
        ...
        var str = '<table border=0><tbody><tr><td valign="top"><span id="j_id14"><label for="methodSel" class="LblLev2Txt_sun4">'+
                            '<span>テストするメソッドを選択します: </span></label></span></td>';
        ...
        str += "</select></span></td><td width=46/><td><a class='Btn1_sun4 Btn1Hov_sun4' onclick='ts.testResource()'>テスト</a></td></tr></tbody></table>";
        return str;
    },
    ...
    doShowDynamicResource : function (uri, mName, mediaType) {
        ...
        str += "</select></span></td>";
        str += "<td width=30/>"
        str += "<td><span id=j_id14><a class='Btn2_sun4 Btn1Hov_sun4' onclick='ts.addParam()'>パラメータを追加</a>";
        str += "</span></td><td><a class='Btn1_sun4 Btn1Hov_sun4' onclick='ts.testResource()'>テスト</a></td></tr></tbody></table><br/>";
        ...
        this.updatepage('request', '<span class=bld>リソース:</span> '+uriLink+' <br/>(<a href="'+req+'" class=Hyp_sun4 target="_blank"><span>'+this.getDisplayURL(req, 90)+'</span></a>)');
    },
    ...
    clearAll : function() {
        this.clearOutput();
        this.updatepage('request', 'ナビゲーションバー (このページの左側) 上でテストするノードを選択します。');
    ...
    getParamRep : function (req, mName) {
        ...
        if(mName == 'PUT' || mName == 'POST') {   
            str += '<tr><td valign="top"><span id="j_id14"><label for="blobParam" class="LblLev2Txt_sun4">'+
                ...
                '<td><span id="j_id14"><textarea class="TxtAra_sun4 TxtAraVld_sun4" id=blobParam name=params rows=6 cols=65>ここにコンテンツを挿入します。</textarea></span></td></tr>';
        }
        str += '</tbody></table>';
        return str;
    },
    ...
    createIFrameForUrl : function (url) {
        var c = 
            '<iframe id="iFrame_" src="'+url+'" class="frame" width="'+ts.iframeWidth+'" align="left">'+
                '<p>参照 <a class=Hyp_sun4 href="'+url+'">"'+url+'"</a>.</p>'+
            '</iframe>';
        return c;
    },
    ...
    monitor : function (method, xmlHttpReq, param) {
        ...
        var s = '要求: ' + this.currentMethod + ' ' + cURL + '\n<br/>' + params +
                    '\n\n<br/><br/>状態: ' + xmlHttpReq.status + ' (' + xmlHttpReq.statusText + ')'+
                    '\n\n<br/><br/>タイムスタンプ: ' + ' ' + xmlHttpReq.getResponseHeader('Date') + '';
        var prevs = '';
    ...
    updateContent : function (content) {
        var showRaw = true;
        var cErr = 'コンテンツにコンテナ-被コンテナ関係がない可能性があります。コンテンツについては、raw データのビューを参照してください。';
    ...
    createTabs : function(tableContent, rawContent, showRaw) {
        ...
        if(subResources == null)
            subResources = '利用可能なサブリソースがありません。';
        var tabs = '<br/><span class=bld>状態:</span> '+ this.currentXmlHttpReq.status+' ('+this.currentXmlHttpReq.statusText+')<br/><br/>'+
    ...
    getTab : function (id, actived) {
        ...
        if(actived)
            return '<td class="Tab1TblSelTd_sun4"><div title="現在の選択: テキストフィールド" class="Tab1SelTxtNew_sun4"><a name="selectedTabAnchor" id="tab'+id+'"/>'+name+'</div></td>';
        else {
            if(id == '')
    ...
    debug : function(message) {
        ...
        if(dbgComp == null) {
            ...
            dbgComp.innerHTML = '<table><tr><td><span style="color: blue">Rest デバッグウィンドウ</span></td><td>'+addActionStr + '</td></tr></table><br/>';
        }
    ...
    getHttpRequest : function() {
        ...
        {    // Internet Explorer 6.0+, 5.0+
            ...
            catch (e)
            {
                ...
                {
                    ts.debug("ブラウザで AJAX がサポートされていません。");
                }
            }
        }
        return xmlHttpReq;
        ...
        try {
            ...
            if (this.isResponseReady(method, xmlHttpReq, content, monitor)) {
              ...
              if(rtext == undefined || rtext == '' || rtext.indexOf('HTTP Status') != -1) {
                  var err = method+' 要求Failed RequestFailed --> 状態: (' + status+')\n<br/>'+
